...
所以，既然 eBPF 提供这么强大的、高效的网络管理机制，而 Cilium 又有能力做到了 L47 层的网络管理，那就作为 Istio 的数据层提供了可能，只不过从当前 Cilium 看，可能只能做到转发层的业务，只能在网络栈以 hook 的方式执行代码，并不能新建网络连接，从而像流量的统计主动上报、SSL 的卸载这样的功能还无法实现，即便如此，依旧是一个有趣和有价值的项目，我们继续期待。
