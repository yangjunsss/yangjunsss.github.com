---
layout: post
title: Service Mesh利用 BPF 实现另一种更高效的代理探索
categories: [general]
tags:
fullview: false
...
Service Mesh 提供了集群服务化开发的新思路，核心主要是构建一个代理转发网络结合控制和转发分离的思想来做集群应用调度管理，而 Linux Kernel 提供一种运行时高效扩展和高效转发的网络代理机制 BPF，并能实现 L47 层代理转发，假如借助 BPF，并应用到 Service Mesh 的数据转发层，作为一个新组件，对接 Pilot、Mixer 控制面，实现控制层的策略和流量管理，是否是一种新方式？而且这种方式会极大提高性能，虽然性能未必是 Mesh 首要考虑的问题，倒不妨是一种有趣的新尝试。
